...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets, scores):
    ...
    classification_loss = torch.sum(scores*F.cross_entropy(class_logits, labels, reduction='none'))/labels.numel()
    ...
    box_loss = torch.sum(torch.unsqueeze(scores[sampled_pos_inds_subset], dim=1)*F.smooth_l1_loss(
...
class RoIHeads(nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels, gt_scores):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image, gt_scores_in_image in zip(proposals, gt_boxes, gt_labels, gt_scores):
            ...
            if gt_boxes_in_image.numel() == 0:
                ...
                scores_in_image = torch.ones_like(labels_in_image, device=device)
            else:
                ...
                scores_in_image = (gt_scores_in_image[clamped_matched_idxs_in_image]-self.tau_l[labels_in_image])/(self.tau_h-self.tau_l[labels_in_image])
                # scores_in_image.clamp_(max=1.)
                scores_in_image[bg_inds] = 1.
                scores_in_image[ignore_inds] = 0.
    ...
    def select_training_samples(
        ...
        gt_scores = [t["scores"] for t in targets]
    ...
    def forward(
        ...
        if self.training:
            ...
            loss_classifier, loss_box_reg = fastrcnn_loss(class_logits, box_regression, labels, regression_targets, scores)
